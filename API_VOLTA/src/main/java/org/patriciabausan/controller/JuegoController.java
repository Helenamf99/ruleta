...
public class JuegoController {
    ...
    public ResponseEntity<?> eliminarUsuario(@PathVariable Integer id, @RequestHeader("Authorizated") String token) {
        try {
            // 🔒 Si en el futuro queréis validar los permisos reales del admin por base de datos,
            // solo tenéis que quitar las barras de comentario '//' de estas líneas de abajo:
            //
            // if (!juegoService.esAdmin(token)) {
            //     return ResponseEntity.status(HttpStatus.FORBIDDEN)
            //             .body(Map.of("mensaje", "Error: No tienes permisos de administrador"));
            // }
