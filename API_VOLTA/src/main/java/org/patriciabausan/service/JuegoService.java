...
public class JuegoService {
    ...
    private void enviarCorreoReal(String emailDestino, String token) {
        try {
            ...
            String enlace = "https://voltagame.netlify.app/frontend/restablecer.html?token=" + token;
    ...
    // ============ 📧 MÉTODO AUXILIAR: Correo de notificación por cuenta eliminada ============
    private void enviarCorreoEliminacionReal(String emailDestino, String nombreUsuario) {
        try {
            MimeMessage mensaje = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mensaje, true, "UTF-8");

            helper.setTo(emailDestino);
            helper.setSubject("Aviso importante: Tu cuenta en Volta Game ha sido eliminada");
            helper.setFrom(new InternetAddress("ema.martin.19@gmail.com", "Volta Game 🕹️"));

            String contenido = "Hola " + nombreUsuario + ",\n\n"
                    + "Te informamos desde el equipo de administración que tu cuenta de usuario asociada "
                    + "al correo (" + emailDestino + ") ha sido eliminada permanentemente de nuestra plataforma por infracción de términos y condiciones.\n\n"
                    + "A partir de este momento, ya no podrás acceder al juego, a tu panel ni conservar tus estadísticas de juego.\n\n"
                    + "Si consideras que esto ha sido un error, por favor ponte en contacto con los desarrolladores.\n\n"
                    + "Un saludo,\n"
                    + "El equipo de administración de Volta Game.";

            helper.setText(contenido);
            mailSender.send(mensaje);

            System.out.println("🚀 ¡Correo de notificación de eliminación enviado con éxito a: " + emailDestino + "!");

        } catch (Exception e) {
            System.err.println("❌ Error crítico en el SMTP de Brevo al notificar eliminación: " + e.getMessage());
            // No lanzamos RuntimeException aquí para evitar que un fallo en el correo bloquee la eliminación física en la BDD
        }
    }
    ...
    // ============ MÉTODO: Eliminar usuario y notificar por email ============
    public Usuario eliminarYNotificar(Integer id) {
        ...
        // 2. Despachamos el correo usando el nuevo método auxiliar antes de borrarlo
        enviarCorreoEliminacionReal(u.getEmail(), u.getNombreUsuario());

        // 3. Lo purgamos físicamente de la base de datos
        usuarioRepository.delete(u);

        // 4. Devolvemos el objeto eliminado para que el controlador responda correctamente
        return u;
    }
