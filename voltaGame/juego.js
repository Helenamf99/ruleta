...
const sonidoUnknown = document.getElementById("sonidoUnknown");
...
function desbloquearAudio() {
  const audios = [sonidoCorrecto, sonidoError, sonidoRuleta, sonidoUnknown];
...
function crearPanelInfoUsuario() {
  ...
  document
    .getElementById("btn-abandonar-partida")
...
function solicitarAbandonoPartida() {
  ...
  document
    .getElementById("btn-confirmar-salir")
    .addEventListener("click", () => {
  ...
  document.getElementById("btn-denegar-salir").addEventListener("click", () => {
    ...
    if (modalPregunta.style.display === "flex") {
      const elementoRelojModal = document.getElementById(
        "reloj-pregunta-modal",
      );
      relojPregunta = setInterval(() => {
        tiempoPreguntaRestante--;
        if (elementoRelojModal)
...
btnGirarRuleta.addEventListener("click", () => {
  // 1. ARRANCAR AUDIO INMEDIATAMENTE (Evita el lag de inicio)
  sonidoRuleta.volume = 1; 
  sonidoRuleta.currentTime = 0;
  sonidoRuleta.play().catch(err => console.warn("No se pudo reproducir la ruleta:", err));
  ...
  // Aplicamos la rotación un instante después de lanzar el audio
  ruletaDisco.style.transform = `rotate(${gradosAcumulados}deg)`;

  // 2. ANTICIPAR EL FADE OUT (Adelantado a los 2600ms para que no termine tarde)
  setTimeout(() => {
    let intervaloFade = setInterval(() => {
      if (sonidoRuleta.volume > 0.16) {
        sonidoRuleta.volume -= 0.16; // Baja el volumen un 15% cada 100ms (un poco más rápido)
      } else {
  ...
  }, 2400); // Antes estaba en 3000ms. Al ponerlo en 2600ms empezará a decaer bastante antes.
...
async function cargarPreguntaAPI(letra, categoriaRuleta) {
  try {
    ...
    if (preguntasFiltradas.length === 0) {
      console.warn(
        `No hay preguntas en BD para la letra ${letra} en ${categoriaRuleta}. Usando fallback.`,
      );
      preguntaFinal = {
    ...
    modalPregunta.style.display = "flex";
    const opcionesMezcladas = [...preguntaFinal.opciones].sort(
      () => Math.random() - 0.5,
    );
    ...
    modalPregunta.style.display = "flex";
    preguntaDinamica.innerHTML = `
        <h3 style="color:#fe0851;"> Error de Conexión</h3>
...
window.manejarClickRespuesta = function (
  botonPulsado,
  letra,
  respuestaUsuario,
  respuestaCorrecta,
  ...
  const todosLosBotones = document.querySelectorAll(
    ".btn-opcion-juego, .btn-volta",
  );
  todosLosBotones.forEach((btn) => (btn.disabled = true));

  if (respuestaUsuario === "VOLTA") {
    sonidoUnknown.currentTime = 0;
    sonidoUnknown.play().catch(err => console.warn("No se pudo reproducir el sonido de pasapalabra:", err));
    modalPregunta.style.display = "none";
